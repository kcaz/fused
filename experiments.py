...
def plot_betas_scad(net_path):
    ...
    settings_fs=fr.get_settings({'s_it': 10, 'per':((1/(1+float(0.5)))*100), 'return_cons':True})
...
#like plot_beta_diffs, but also plots delta beta against lamS
def deltb_lams(out=None, B0=None, B1=None, scale=1.0):
    lamP = 1
    lamR = 0.5
    lamS = 0.5
    N = 200
    ...
    (beta1, genes1, tfs1) = ds.load_network(os.path.join(out, 'beta1'))
    (beta2, genes2, tfs2) = ds.load_network(os.path.join(out, 'beta2'))

    deltabetas = []
    lamSs = []
    for i in new_fuse_cons:
        (coeff1, coeff2, lamS) = i  
        B1 = Bs_fs[coeff1.sub][coeff1.r, coeff1.c]
        B2 = Bs_fs[coeff2.sub][coeff2.r, coeff2.c]
        #B1 = beta1[coeff1.r, coeff1.c]
        #B2 = beta2[coeff2.r, coeff2.c]
        #B1 = Bs_uf[coeff1.sub][coeff1.r, coeff1.c]
        #B2 = Bs_uf[coeff2.sub][coeff2.r, coeff2.c]
        deltabeta = abs(B1-B2)
        deltabetas.append(deltabeta)
        lamSs.append(lamS)

    plt.scatter(deltabetas, lamSs)
    plt.show()
...
def lamS_dist_fig():
    ...
    for i, N in enumerate(orth_err):
        ...
        ds.write_fake_data1(N1 = 5*70, N2 = 5*70, out_dir = out, tfg_count1=(N_TF, N_G), tfg_count2 = (N_TF, N_G), pct_fused = amt_fused, orth_falsepos = N, measure_noise1 = 0.1, measure_noise2 = 0.1, sparse=0.5, fuse_std = 0.1)
        lamR = 2
        lamP = 1.0 #priors don't matter

        for j, lamS in enumerate(lamSs):

            scad_settings = fr.get_settings({'s_it':2, 'per':((1/(1+float(N)))*100), 'return_cons' : True})
            (errd1, errd2) = fg.cv_model_m(out, lamP=lamP, lamR=lamR, lamS=lamS, k=5, solver='solve_ortho_direct_scad', reverse = False, settings = scad_settings, cv_both = (True, True))
...
def test_iter1(sc=1):
    ...
    (b1d, b2d) = fg.fit_model(out, 1.0, 0.1, 5.5, solver='solve_ortho_direct')
    si = fr.get_settings({'it':10})
    (b1i, b2i) = fg.fit_model(out, 1.0, 0.1, 5.5, solver='iter_solve', settings=si)
    print 'iterative vs. direct: %f' % ((b1d - b1i)**2).mean()
    print 'unfused vs. direct: %f' % ((b1d - b1n)**2).mean()
...
def test_iter_all(k=2, lamP=1.0, lamR=0.5, lamS=1.0):
    ...
    for i in range(len(si_all['lamS_scores'])):
        for j in range(len(si_all['lamS_scores'][i])):
            dic['subject'].append(i)
            dic['species'].append('all')
            dic['MSE'].append(si_all['lamS_scores'][i][j]/si_all['lamS_scores'][i][0])
            dic['lamS'].append(si_all['lamS_path'][i][j])
    for i in range(len(si_su_eu['lamS_scores'])):
        for j in range(len(si_su_eu['lamS_scores'][i])):
            dic['subject'].append(i)
            dic['species'].append('subt-subt_eu')
            dic['MSE'].append(si_su_eu['lamS_scores'][i][j]/si_su_eu['lamS_scores'][i][0])
            dic['lamS'].append(si_su_eu['lamS_path'][i][j])
    for i in range(len(si_su_an['lamS_scores'])):
        for j in range(len(si_su_eu['lamS_scores'][i])):
            dic['subject'].append(i)
            dic['species'].append('subt-anth')
            dic['MSE'].append(si_su_an['lamS_scores'][i][j]/si_su_an['lamS_scores'][i][0])
            dic['lamS'].append(si_su_an['lamS_path'][i][j])

    df = pd.DataFrame(dic)
    sns.tsplot(time='lamS',value='MSE',unit='subject',condition='species',data=df)
...
def test_iter_rd():
    out = 'data/bacteria_standard'
    si = fr.get_settings({'it':10, 'iter_eval':True})
    lamR = 0.2
    lamS = 1.0
    lamP = 1.0
    k = 15
...
def operons_param():
    ...
    for i in range(it):
        ...
        for i in range(k):
            ...
            scores['mse 2'].append(sc[1]['mse'][i][0])
            scores['aupr 1'].append(sc[0]['aupr'][i][0])
            scores['aupr 2'].append(sc[1]['aupr'][i][0])            

    return scores
    

    #cv_model_m(net_path, lamP, lamR, lamS, k, solver='solve_ortho_direct',settings = None, reverse=False, cv_both=(True,True), exclude_tfs=True, pct_priors=0, seed=None, verbose=False, orth_file=['orth'], orgs=None, lamS_opt = None):
...
def operons_scad():
    ...
    gene_to_inds = map(lambda o: {genes[o][x] : x for x in range(len(genes[o]))}, range(len(organisms)))
    tf_to_inds = map(lambda o: {tfs[o][x] : x for x in range(len(tfs[o]))}, range(len(organisms)))

    orth_fn2 = os.path.join(net_path, 'orth')
    orth2 = ds.load_orth(orth_fn2, organisms)
    all_orth = orth+orth2
    settings_fs=fr.get_settings({'s_it': 5, 'per':85, 'return_cons':True})
    #settings_fs=fr.get_settings({'s_it': 5, 'per':((1/(1+float(0.5)))*100), 'return_cons':True})
    Bs_fs = fr.solve_ortho_direct_scad(organisms, genes, tfs, Xs, Ys, all_orth, priors, lamP, lamR, lamS, settings = settings_fs)
    lamS = 0
    Bs_uf = fr.solve_ortho_direct(organisms, genes, tfs, Xs, Ys, all_orth, priors, lamP, lamR, lamS)


    new_fuse_cons = settings_fs['cons'] 

    operons_dict = dict()
    #creates a dictionary mapping gene to operon
    dist_S = dict()
    #creates a dictionary mapping distance (unit is number of genes in between pair in the operon + 1) to lamS
    dist_lamS = dict() 
    dist_lamS['distance']=[]
    dist_lamS['lamS']=[]
    ...
    for i in range(len(ops)):
        genes_op = ops[i].split('\t')[0].split(',')
        for gene in genes_op:
            operons_dict[gene] = genes_op
    ...
    for j in new_fuse_cons:
        (coeff1, coeff2, lamS) = j
        g1 = genes1[coeff1.c]
        g2 = genes1[coeff2.c]
        if g1 in operons_dict:
            if g2 in operons_dict[g1]:
                if coeff1.c == coeff2.c and coeff1.r == coeff2.r:
                    print 'self fusion constraint!'
                dist = abs(operons_dict[g1].index(g1) - operons_dict[g1].index(g2))
                ind = min(dist, 3)
                B1 = Bs_fs[coeff1.sub][coeff1.r, coeff1.c]
                B2 = Bs_fs[coeff2.sub][coeff2.r, coeff2.c]
                ...
                Buf2 = Bs_uf[coeff2.sub][coeff2.r, coeff2.c]
                Bufs_plot[ind].append(Buf1)
                Bufs_plot[ind].append(Buf2)
                Bufdiff = abs(Buf1-Buf2)
                #if Bufdiff > 0.001:
    ...
    for k in range(len(dist_S)):
        new_file.write(str(dist_S.keys()[k]) + '\t' + '\t'.join(map(str, dist_S[dist_S.keys()[k]])) + '\n')
    new_file.close()
    ...
    #    s = pd.Series(lamSs, index=[m]*len(lamSs), name='lamS')
    #    pdlist.append(s)

    #df=pd.concat(pdlist).reset_index()
    sns.pointplot(x="distance",y="lamS",ci=95,data=df)
    plt.show()

    print len(Bsdiff_plot[1])
    print len(Bsdiff_plot[2])
    print len(Bsdiff_plot[3])

    plt.figure(1)
    plt.subplot(231)
    plt.hist(Bufs_plot[1],100)
    plt.ylim(-5,200000)
    plt.xlim(-15,15)
    plt.title("unfused Bs dist 1")

    plt.subplot(232)
    plt.hist(Bufs_plot[2],100)
    plt.ylim(-5,200000)
    plt.xlim(-15,15)
    plt.title("unfused Bs dist 2")
    
    plt.subplot(233)
    plt.hist(Bufs_plot[3],100)
    plt.ylim(-5,200000)
    plt.xlim(-15,15)
    plt.title("unfused Bs dist 3+")

    plt.subplot(234)
    plt.hist(Bs_plot[1], 100)
    plt.ylim(-5,200000)
    plt.xlim(-15,15)
    plt.title("fused Bs dist 1")

    plt.subplot(235)
    plt.hist(Bs_plot[2], 100)
    plt.ylim(0,200000)
    plt.xlim(-15,15)
    plt.title("fused Bs dist 2")

    plt.subplot(236)
    plt.hist(Bs_plot[3], 100)
    plt.ylim(0,200000)   
    plt.xlim(-15,15) 
    plt.title("fused Bs dist 3+")
    plt.show()

    plt.scatter(Bufdiff_plot[1], Bsdiff_plot[1], alpha=0.5)
    plt.xlabel('deltabetas, before fusion')
    plt.ylabel('deltabetas, after fusion')
    plt.xlim(0,15)
    plt.ylim(0,15)
    plt.title('distance 1')
    plt.show()

    plt.scatter(Bufdiff_plot[2], Bsdiff_plot[2], alpha=0.5)
    plt.xlabel('deltabetas, before fusion')
    plt.ylabel('deltabetas, after fusion')
    plt.title('distance 2')
    plt.show()

    plt.scatter(Bufdiff_plot[3], Bsdiff_plot[3], alpha=0.5)
    plt.xlabel('deltabetas, before fusion')
    plt.ylabel('deltabetas, after fusion')
    ...
    plt.figure(1)
    plt.subplot(121)
    plt.hist(Bufdiff_plot[1], 100)
    plt.xlim(0,25)
    plt.ylim(-5,300000)
    plt.title("deltabetas dist 1, before fusion")

    plt.subplot(122)
    plt.hist(Bsdiff_plot[1], 100)
    plt.xlim(0,25)
    plt.ylim(-5,300000)
    plt.title("deltabetas dist 1, after fusion")

    plt.show()

    plt.figure(1)
    plt.subplot(122)
    plt.hist(Bsdiff_plot[2], 100)
    plt.xlim(0,14)
    plt.ylim(-5,140000)
    plt.title("deltabetas dist 2, after fusion")

    plt.subplot(121)
    plt.hist(Bufdiff_plot[2],100)
    plt.xlim(0,14)
    plt.ylim(-5,140000)
    plt.title("deltabetas dists 2, before fusion")
    plt.show()

    plt.figure(1)
    plt.subplot(122)
    plt.hist(Bsdiff_plot[3], 100)
    plt.xlim(0,14)
    plt.ylim(-5,300000)
    plt.title("deltabetas dist 3+, after fusion")

    plt.subplot(121)
    plt.hist(Bufdiff_plot[3],100)
    plt.title("deltabetas dist 3+, before fusion")
    plt.xlim(0,14)
    plt.ylim(-5,300000)
    plt.show()

    Bufdiff_toplot = [item for sublist in Bufdiff_plot for item in sublist]
    lamS_toplot = [item for sublist in lamS_s for item in sublist]
    color1 = np.random.rand(4)
    colors = []
    for i in range(len(color1)):
        colori = [color1[i]]*len(Bufdiff_plot[i])
        colors.extend(colori)

    print len(Bufdiff_toplot)
    print len(lamS_toplot)
    print len(colors)
    plt.scatter(Bufdiff_toplot, lamS_toplot, c=colors, alpha = 0.5)
...
def xspecies_perf(lamP=(0.03,0.05), lamR=(0.368,0.5), lamSs=[0,0.5,1], k=10,cv_both=(True,False), orgs=['B_subtilis','B_anthracis'], orth_file=['orth']):
    ...
    for i in range(len(lamSs)):
        errd = fg.cv_model_m(out, lamP=lamP, lamR=lamR, lamS=lamSs[i], k=k, solver=solver, settings=settings, reverse=True, cv_both=cv_both, exclude_tfs=False, seed=seed, orgs=orgs, lamS_opt=None, orth_file=orth_file)[0]    
        errdf.append(errd)
