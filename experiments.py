...
def test_opt():
    ...
    if not os.path.exists(os.path.join('data','fake_data','test_opt')):
        os.mkdir(os.path.join('data','fake_data','test_opt'))
    #iterate over how much fusion

    f = 5
    out1 = os.path.join('data','fake_data','test_opt','dat_'+str(f))
    if not os.path.exists(out1):
        os.mkdir(os.path.join('data','fake_data','test_opt','dat_'+str(f)))
    ...
    for num_species in range(2):
        dsi = ds.standard_source(out1,num_species)
        dss.append(dsi)
    ...
    orth_fn = os.path.join(out1, 'orth')
    orth = ds.load_orth(orth_fn, organisms)
    priors = dss[0].get_priors()[0]+dss[1].get_priors()[0]
    it = 5
    ...
    for fold in range(k):
        ...
        lamS_opt = None
        settings = None
        ridge_con = fr.priors_to_constraints(organisms, genes, tfs, priors, lamP*lamR)
        fuse_con = fr.orth_to_constraints(organisms, genes, tfs, orth, lamS, lamS_opt)

        Bss = fr.iter_solve(Xs, Ys, fuse_con, ridge_con, lamR, it)
        Bs = fr.solve_ortho_direct(organisms, genes, tfs, Xs, Ys, orth, priors, lamP, lamR, lamS, lamS_opt, settings = settings)
        print abs(Bss[0][0]-Bs[0]).sum()
