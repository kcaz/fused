...
def plot_bacteria_performance_opt_param():
    ...
    rseed = random.random()
    for i, lamS in enumerate(lamSs):
        (aupr, best_lamP, best_lamR, best_lamS, grid) = fg.grid_search_params(data_fn, [lamP], lamRs, [lamS], k, solver='solve_ortho_direct',special_args=None, reverse=False, cv_both=(True,False), exclude_tfs=False, eval_metric='aupr')
        aupr_array[i,0] = aupr
...
def plot_bacteria_performance_priors2():
    ...
    lamS = 0
    k = 10
    pct_priors = np.arange(0,0.8,0.2)

    out = 'data/bacteria_standard'
    metrics = ['mse','R2','aupr','auc','corr', 'auc_con','aupr_con']

    no_pr = {m : np.zeros((k, len(pct_priors))) for m in metrics} #subtilis
    yes_pr = {m : np.zeros((k, len(pct_priors))) for m in metrics} #subtilis
    rseed = random.random()
