...
def test_iter1(sc=1):
    ...
    (b1d, b2d) = fg.fit_model(out, 1.0, 0.1, lamS, solver='solve_ortho_direct')
    si = fr.get_settings({'it':100})
    (b1i, b2i) = fg.fit_model(out, 1.0, 0.1, lamS, solver='iter_solve', settings=si)
...
def test_iter1m(sc=1):
    ...
    si = fr.get_settings({'it':30, 'iter_eval':True})
    k=15
    lamP = 1.0
    lamR = 0.5
    errd = fg.cv_model_m(out, lamP=lamP, lamR=lamR, lamS=lamS, k=k, solver='iter_solve', reverse = True, cv_both = (True, False), exclude_tfs=False, pct_priors=0, seed=44.44, settings=si)
    return errd
...
def test_iter_rd(k=15):
    out = 'data/bacteria_standard'
    si = fr.get_settings({'it':20, 'iter_eval':True})
    lamR = 0.5
    lamS = 1.0
    lamP = 1.0
    
    errd = fg.cv_model_m(out, lamP=lamP, lamR=lamR, lamS=lamS, k=k, solver='iter_solve', reverse = True, cv_both = (True, True, True), exclude_tfs=False, pct_priors=0, seed=44.44, settings=si, orgs=['B_subtilis','B_anthracis'])
    
    plt.plot(si['sol_changes'][0])
    plt.show()
    return si

def test_iter_all(k=15, lamR=0.5, lamS=1.0):
    out = 'data/bacteria_standard'
    sia = fr.get_settings({'it':20, 'iter_eval':True})
    sies = fr.get_settings({'it':20, 'iter_eval':True})
    sisa = fr.get_settings({'it':20, 'iter_eval':True})

    lamP = 1.0
    
    errda = fg.cv_model_m(out, lamP=lamP, lamR=lamR, lamS=lamS, k=k, solver='iter_solve', reverse = True, cv_both = (True, True, True), exclude_tfs=False, pct_priors=0, seed=44.44, settings=sia, orgs=['B_subtilis','B_anthracis','B_subtilis_eu'])
    errdes = fg.cv_model_m(out, lamP=lamP, lamR=lamR, lamS=lamS, k=k, solver='iter_solve', reverse = True, cv_both = (True, True, True), exclude_tfs=False, pct_priors=0, seed=44.44, settings=sies, orgs=['B_subtilis','B_subtilis_eu'])
    errdsa = fg.cv_model_m(out, lamP=lamP, lamR=lamR, lamS=lamS, k=k, solver='iter_solve', reverse = True, cv_both = (True, True, True), exclude_tfs=False, pct_priors=0, seed=44.44, settings=sisa, orgs=['B_subtilis','B_anthracis'])
    import pickle
    with file('lamSpaths') as f:
        pickle.dump((sia, sies, sisa), f)
