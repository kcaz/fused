...
def load_orth(orth_fn, organisms):
    ...
    for o in fsnt:
        ...
        genes = []
        for org in range(len(o)):
            genes.append(map(lambda n: fr.one_gene(name = n, organism = organisms[org]), filter(len, o[org].split(','))))
...
#priors as input is a list of tuples containing gene/tf names
    ...
    (bs_e, bs_t, bs_genes, bs_tfs) = sub.load_data()
    (bseu_e, bseu_t, bseu_genes, bseu_tfs) = sub.load_data()
    ...
    def orth_sub(bs_genes, bseu_genes, bs_tfs, bseu_tfs):
        ...
        for g in bs_genes:
            if g in bseu_genes:
                print g
                orth = fr.orthology(genes = (fr.one_gene(name=g,organism=organisms[0]), fr.one_gene(name=g, organism=organisms[1])), real = real)
                orths.append(orth)
        return orths

    (bs_priors, bs_sign) = sub.get_priors()
    (bseu_priors, bseu_sign) = sub.get_priors()
