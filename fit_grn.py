...
def fit_model(data_fn, lamP, lamR, lamS, solver='solve_ortho_direct', settings = None, orth_file ='orth'):
    ...
    if solver == 'iter_solve':
        ...
        Bs = fl.solve_ortho_iter(organisms, genes, tfs, Xs, Ys, orth, priors, lamP, lamR, lamS, settings = settings)[-1]
...
def cv_unfused(data_fn, lamP, lamR, k, solver='solve_ortho_direct', settings=None, reverse=False, cv_both=(True,True), exclude_tfs=True, pct_priors=0, seed=None, verbose=False, orth_file='orth', orgs=None, lamS_opt=None):
    ...
    for fold in range(k):
        ...
        chance = compute_chance_precision(rc_B, rc_genes, rc_tfs, priorste, tr_priors=priorstr, exclude_tfs=exclude_tfs, constraints = None, sub = si)
        err_dict['chance'][fold, 0] = chance
        
        #top 100 interactions
        err_dict['top_100'] = top_k_interactions(rc_B, rc_genes, rc_tfs, priorste, org=organisms[si], k=np.inf)
